...
def convert_call_site(site, funct, asm_line, asm_dest, 
    ...
    if not indirect_call:
        ...
        increment_cdi_funct(functs, funct, 2) #or 2?
        return
    ...
    call_sled += '1:\n' # 8 complete lines
    cdi_abort_line, cdi_abort_newlines = cdi_abort_str(sled_id_faucet(), funct.asm_filename, 
            options['--no-abort-messages'], dwarf_loc)
    call_sled += cdi_abort_line
    call_sled += '2:\n'
    
    increment_cdi_funct(functs, funct, 9 + cdi_abort_newlines)
...
def convert_return_site(site, funct, asm_line, asm_dest, cfg,
    ...
    for target_label, multiplicity in site.targets.iteritems():
        ...
        while i <= multiplicity:
            ...
            i += 1
    # 3*multiplicity + 1
    cdi_abort_line, n = cdi_abort_str(sled_id_faucet(), funct.asm_filename,
        options['--no-abort-messages'], dwarf_loc)
            
    ret_sled += cdi_abort_line
    n +=  2 * (multiplicity-1) + 1
    increment_cdi_funct(functs, funct, n)
